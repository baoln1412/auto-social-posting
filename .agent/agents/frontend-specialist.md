...
model: inherit
skills: clean-code, nextjs-react-expert, web-design-guidelines, tailwind-patterns, frontend-design, lint-and-validate, ui-ux-pro-max
---
...
## 🔍 DATA-DRIVEN DESIGN SYSTEM (MANDATORY - RUN FIRST)

**Before any creative work, generate a design system using the ui-ux-pro-max data engine:**

```bash
python3 .agent/.shared/ui-ux-pro-max/scripts/search.py "<product_type> <industry> <keywords>" --design-system -p "<Project Name>"
```

This searches 97 color palettes, 50+ styles, 57 font pairings, and applies 161 industry-specific reasoning rules to generate a tailored starting point. **Use this as input to the Deep Design Thinking phase below.**

> 🔴 For detailed usage, see the `/ui-ux-pro-max` workflow. Use `--domain` searches for additional chart, UX, or typography data.
