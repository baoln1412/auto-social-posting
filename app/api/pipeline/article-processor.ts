...
function buildContentPrompt(article: Article): string {
  return [
    ...
    'Include: WHO (names, ages, relationships), WHAT happened (sequence of events),',
    'WHEN and WHERE it occurred, HOW it was discovered, key EVIDENCE cited by police/prosecutors,',
    'CHARGES filed, and any COMMUNITY REACTION or investigation status.',
...
function buildBatchContentPrompt(articles: Article[]): string {
  ...
  return [
    ...
    'Include: WHO, WHAT, WHEN, WHERE, EVIDENCE, CHARGES, community reaction.',
